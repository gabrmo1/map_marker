...
const deleteMarkers = async (req, res) => {
  ...
  try {
    ...
    if (marker) {
      marker.destroy()
        .then(() => res.status(200).json({ success: 'Marcador removido' }));
    } else {
      res.status(500).json({ error: `Ocorreu um erro ao buscar o marcador${id} para remover` });
    }
  } catch {
    res.status(500).json({ error: `Ocorreu um erro ao remover o marcador${id}` });
  }
  return res;
};
